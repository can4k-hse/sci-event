...
## Header
- Левая часть: логотип — название конференции через `<Text>` (берём `event.name` из `src/mocks/event.ts`)
- Правая часть: навигационные ссылки через `NavLink` из `react-router-dom`
  - «Главная» → `/`
  - «Программа» → `/schedule`
  - «Спикеры» → `/speakers`
  - «Войти» → `/login` (или `{user.name} {user.surname}` если `isAuthenticated`)
...
## Зависимости
- TICKET-03 (event mock) — для названия конференции в логотипе
